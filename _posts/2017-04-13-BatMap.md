...
Dolayısıyla Tuple'ın GetHashCode uygulamasını ~~kullanan bir struct yazarak (MapPair)~~ *ile ürettiğimiz int değeri kullanarak* önemli bir performans artışı sağlayabiliyoruz. Aşağıda Dictionary ve HashCode üreten kodu görebilirsiniz:
{% highlight csharp %}
