...
* Bir de .Count() metoduna değinelim. Genelde IEnumerable üzerinden yapılan .Count çağrısının tüm listeyi gezdiği düşünülür. Ancak karşılaşacağımız çoğu senaryoda aslında bu listeler bir List, Collection ya da bu gibi bir Concrete tiptir. Bu durumda .Count çağrısı arka planda çok daha hızlı olan .Count property erişimiyle sağlanır ve bir performans kaybı yaşanmaz.
